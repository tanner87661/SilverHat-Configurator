...
var HWTemplate = {"Module":0,"PwrPin":0,"AnalogPin":0,"SensorFactor":0.00,"ARPin":0,"SCCurrent":0,"Vals":[0,0,0,0]};
...
//	ws.send("{\"Cmd\":\"BST\", \"SV\":" + SVCmd + "}");
	var outMsg = {"Cmd":"SV","Prm":{"Dst":dst, "Opc": sv_cmd, "Addr":sv_addr, "Vals":[]}};
	switch (sv_cmd)
	{
		case 0x01: //write 1 byte
			{
//				sprintf(txMsg, "{\"Cmd\":\"SV\",\"Prm\":{\"Dst\":%i,\"Opc\":%i,\"Addr\":%i,\"Vals\":[%i]}}\n", dst, sv_cmd, sv_addr, data[0]);
				outMsg.Prm.Vals = [data[0]];
//				console.log(outMsg);
...
//				console.log(outMsg);
		...
		case 0x09: //change LN address
			{
//				sprintf(txMsg, "{\"Cmd\":\"SV\",\"Prm\":{\"Dst\":%i,\"Opc\":%i,\"Addr\":%i,\"Vals\":[%i,%i,%i,%i]}}\n", sv_addr, sv_cmd, svDest, data[0], data[1], data[2], data[3]);
				outMsg.Prm.Dst = sv_addr;
				outMsg.Prm.Vals = [data[0], data[1], data[2], data[3]];
				outMsg.Prm.Addr = dst;
//				console.log(outMsg);
				writeToStream(JSON.stringify(outMsg));
...
function uiDisable (status) {
    document.getElementById('btnLoad').disabled = status
    document.getElementById('btnSave').disabled = status
    document.getElementById('LNAddr').disabled = status
    document.getElementById('numMods').disabled = status
    document.getElementById('cbsetup_0').disabled = status
    document.getElementById('cbsetup_1').disabled = status
    document.getElementById('cbsetup_2').disabled = status
...
//	console.log(DeviceData);
	setVisibility(false, cbUseLN);

	setVisibility(false, devLoconetInputPanel);
	setVisibility(DeviceData.Boosters.length > 0, dataTab);
	setVisibility(false, document.getElementById("layoutstatus"));
//	setVisibility((DeviceData.LNMode > 0) && ([4,5,6].indexOf(configData[nodeCfg].InterfaceIndex) >= 0), document.getElementById("outputstatus"));
	...
	{
		setVisibility(false, document.getElementById("boosterconfig_inp_" + i.toString() + "_2"));
		setVisibility((false) && ([4,5,6].indexOf(configData[nodeCfg].InterfaceIndex) >= 0), document.getElementById("boosterconfig_inp_" + i.toString() + "_3"));
...
function constructPageContent(contentTab)
{
	...
	tempObj = createEmptyDiv(mainScrollBox, "div", "tile-1", "");
		createButton(tempObj, "", "Load Configuration", "btnLoad", "loadSettings(this)");
...
//					cbUseLN.checked = DeviceData.LNMode;
					cbUseLN.checked = false; //no stick, no communication
					for (var j = 0; j < 2; j++)
